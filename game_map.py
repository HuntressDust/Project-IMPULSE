...
class GameMap:
    ...
    def stations(self):
        yield from (entity for entity in self.entities if isinstance(entity, Station))
    ...
    def pixel_in_viewport(self,x,y)->bool:
        o_x, o_y,e_x,e_y,offset = self.get_viewport()

        if x>offset:
            if y>0:
                if x<offset+self.engine.game_world.viewport_width:
                    if y<self.engine.game_world.viewport_height+1:
                        return True
        return False
    def render_tiles (self, console : Console) -> None:
        ...
        if self.engine.player.fighter.hp > self.engine.player.fighter.max_hp//4:
            console.draw_frame(x=offset-1,y=0, width = self.engine.game_world.viewport_width+2,
                               height = self.engine.game_world.viewport_height+2, fg=color.blue, clear=False,)
        else:
            console.draw_frame(x=offset - 1, y=0, width=self.engine.game_world.viewport_width + 2,
                               height=self.engine.game_world.viewport_height + 2, fg=color.red, clear=False, )


    def render_entities(self, console : Console,has_sensor_perk: bool):
        o_x, o_y,e_x,e_y,offset = self.get_viewport()
        ...
        for entity in entities_sorted_for_rendering:
                ...
                elif has_sensor_perk:
                    if self.pixel_in_viewport(entity.x-o_x+offset,entity.y-o_y+1):
                        if entity in self.actors:
                            console.print(x=entity.x - o_x + offset, y=entity.y - o_y + 1, string="!", fg=color.invalid)
...
class GameWorld:
    ...
    def generate_floor(self,floor_type: str="normal" )-> None:
